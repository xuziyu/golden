...
<script>
	function formatOperateColumn(value, row) {
		//----------------------------------------------1 js方法名和参数\--------------------------------------
		return '<a href="#" class="icon-edit" style="display:inline-block;width:40px;"  onclick="edit(\'' + row.tid + '\')">&nbsp;</a>' +
			'<a href="#" class="icon-remove" style="display:inline-block;width:40px;"  onclick="deletem(\'' + row.tid + '\')">&nbsp;</a>'
			;
	}
	//----------------------------------------------2方法名和url参/数--------------------------------------
	function edit(id) {
		
		window.location.href='edit.action?tid=' + id ;
	}
	//----------------------------------------------3方法名和url参/数--------------------------------------
	function deletem(id) {
	...
	<div id="toolbar">
		<form id='formAreaQuery'>
			<div style="padding:6px;padding-top: 15px;">
				...
				<span>手机号码:</span>
				<input id="telephoneNum" style="height:20px;border:1px solid #ccc">
		...
		<table id="dgArea" class="easyui-datagrid" data-options="url:'page_json.action',fit:true,toolbar:'#toolbar',singleSelect:true,collapsible:true,method:'get',pagination:'true',fitColumns:true">
			<thead>
				<tr>
					...
					<th data-options="field:'telephoneNum',width:200,align:'center'">手机号码</th>
					<th data-options="field:'approveDate',width:200,align:'center',formatter:date">认证时间</th>
					<th data-options="field:'approveState',width:200,align:'center',formatter:status">审核是否通过</th>
