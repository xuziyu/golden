...
<script>
	...
	function edit(id) {
		...
		window.location.href = 'edit.action?limitCode=' + id;
	}
	//----------------------------------------------3方法名和url参/数--------------------------------------
	function deletem(id) {
		...
		$.ajax({
			async : true, //是否异步
			type : "post",
			url : 'deleteAj.action?limitCode=' + id,
	...
	<div id="toolbar">
		<form id='formAreaQuery'>
			<div style="padding: 6px; padding-top: 15px;">
				<span>信用额度编号:</span>
				<input id="limitCode" style="height: 20px; border: 1px solid #ccc">
				
				<span>用户id:</span>
				<input id="userId" style="height: 20px; border: 1px solid #ccc">
				
				<span>信用额度:</span>
				<input id="creditLimit" style="height: 20px; border: 1px solid #ccc">
				
				<span>信用等级:</span>
				<input id="creditGrade" style="height: 20px; border: 1px solid #ccc">
	...
	<table id="dgArea" class="easyui-datagrid"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'limitCode',width:180,align:'center'">信用额度编号</th>
				<th data-options="field:'userId',width:200,align:'center'">用户id</th>
				<th data-options="field:'creditLimit',width:200,align:'center'">信用额度</th>
				<th data-options="field:'creditGrade',width:200,align:'center'">信用等级</th>
			</tr>
		</thead>
	</table>
</body>
