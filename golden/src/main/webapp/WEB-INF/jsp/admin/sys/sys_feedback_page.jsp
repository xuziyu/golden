...
<script>
	...
	function edit(id) {
		...
		window.location.href = 'edit.action?feedbackId=' + id;
	}
	//----------------------------------------------3方法名和url参/数--------------------------------------
	function deletem(id) {
		...
		$.ajax({
			async : true, //是否异步
			type : "post",
			url : 'deleteAj.action?feedbackId=' + id,
	...
	function doSearch() {
		$('#dgArea').datagrid({
			queryParams : {
				...
				"feedbackConent" : $('#feedbackConent').val(),
	...
	<div id="toolbar">
		<form id='formAreaQuery'>
			<div style="padding: 6px; padding-top: 15px;">
				<span>反馈人:</span>
				<input id="feedbackName" style="height: 20px; border: 1px solid #ccc">
				
				<span>反馈内容:</span>
				<input id="feedbackConent" style="height: 20px; border: 1px solid #ccc">
				
				<span>反馈时间:</span>
				<input id="feedbackTime" style="height: 20px; border: 1px solid #ccc">
	...
	<table id="dgArea" class="easyui-datagrid"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'feedbackName',width:180,align:'center'">反馈人</th>
				<th data-options="field:'feedbackConent',width:200,align:'center'">反馈内容</th>
				<th
					data-options="field:'feedbackTime',width:200,align:'center',formatter:date">反馈时间</th>
			</tr>
		</thead>
	</table>
</body>
