...
<script>
	...
	function edit(id) {
		...
		window.location.href = 'edit.action?rechargeId=' + id;
	}
	//----------------------------------------------3方法名和url参/数--------------------------------------
	function deletem(id) {
		...
		$.ajax({
			async : true, //是否异步
			type : "post",
			url : 'deleteAj.action?rechargeId=' + id,
	...
	function doSearch() {
		$('#dgArea').datagrid({
			queryParams : {
				...
				"rechargeStatu" : $('#rechargeStatu').val()
			}
		});
	}
</script>
	...
	<div id="toolbar">
		<form id='formAreaQuery'>
			<div style="padding: 6px; padding-top: 15px;">
				<span>流水号:</span>
				<input id="rechargeCode" style="height: 20px; border: 1px solid #ccc">
				<span>用户id:</span>
				<input id="userId" style="height: 20px; border: 1px solid #ccc">
				<span>充值类型:</span>
				<input id="rechargeType" style="height: 20px; border: 1px solid #ccc">
				<span>充值金额:</span>
				<input id="rechargeMoney" style="height: 20px; border: 1px solid #ccc">
				<span>充值时间:</span>
				<input id="rechargeTime" style="height: 20px; border: 1px solid #ccc">
				<span>状态:</span>
				<input id="rechargeStatu" style="height: 20px; border: 1px solid #ccc">
	...
	<table id="dgArea" class="easyui-datagrid"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'rechargeCode',width:180,align:'center'">流水号</th>
				<th data-options="field:'userId',width:180,align:'center'">用户id</th>
				<th data-options="field:'rechargeType',width:180,align:'center'">充值类型</th>
				<th data-options="field:'rechargeMoney',width:180,align:'center'">充值金额</th>
				<th data-options="field:'rechargeTime',width:200,align:'center',formatter:date">充值时间</th>
				<th data-options="field:'rechargeStatu',width:180,align:'center'">状态</th>
			</tr>
		</thead>
	</table>
</body>
